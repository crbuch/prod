async function getOilPrice() {
  ...
  let response = await fetch(
    "https://commodities-api.com/api/latest?access_key=" +
      token +
      "&base=USD&symbols=WTIOIL"
  ); //makes promise to get get response
...
getOilPrice().then(
  (data) =>
    (document.getElementById("WTIOIL").innerHTML =
      "WTI: $" + (1 / data.data.rates.WTIOIL).toFixed(2))
); //calls promise w/ .then changes vaule of HTML id='WTIOIL' rate found w/ 2 decimals
console.log("hi");
...
d3.select("#siteSelection").on("change", function () {
  ...
  if (clickedFromAnalyzed == null) {
    ...
    d3.json("./static/allProductionData.json").then((data) => {
      ...
      if (timeFrame === 0 && scale === "log") {
        //HIDE BUTTONS RELATED TO LINEAR FORMAT
        ...
        //HIDE BUTTONS RELATED TO LOG FORMAT
    ...
    d3.json("./static/cumProd.json").then((cumData) => {
      ...
      document.getElementById("cumCurve").innerHTML =
        "Cum: " + selectedWellCum + " MBBLS, " + selectedWellGasCum + " MMCF";
...
d3.select("#Inception").on("click", function () {
  Curve((timeFrame = 0), (scale = "linear"));
});
...
d3.select("#Days365").on("click", function () {
...
d3.select("#InceptionLog").on("click", function () {
  Curve((timeFrame = 0), (scale = "log"));
});
d3.select("#Days30Log").on("click", function () {
  Curve((timeFrame = 31), (scale = "log"));
});
d3.select("#Days180Log").on("click", function () {
  Curve((timeFrame = 181), (scale = "log"));
});
d3.select("#Days365Log").on("click", function () {
