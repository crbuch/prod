...
d3.select("#siteSelection").on('change', function () { Curve(timeFrame = 0, scale = 'linear'); });

//FUNCTION TO POPULATE CURVE BASED ON ANALYZE CLICK
function ClickedFromAnalyze(timeFrame, scale) {
  ...
  if (clickedFromAnalyzed == null) {
    ...
    d3.json("./static/allProductionData.json").then((data) => {
      ...
      if (timeFrame === 0 && scale === 'log') {  //HIDE BUTTONS RELATED TO LINEAR FORMAT
      ...
      else if (timeFrame === 0 && scale === 'linear') { //HIDE BUTTONS RELATED TO LOG FORMAT
    ...
    d3.json("./static/pumpInfo.json").then((pumpData) => {
      ...
      var pumpingInfoToShow = { "Well Name": "doesn'scale exist because it is not pumping" };
...
function Curve(timeFrame, scale) {
  ...
  d3.json("./static/economics.json").then((economicsData) => {
    ...
    document.getElementById("pnl").innerHTML = "P&L : " + "$" + wellRMPL.toFixed(0).toLocaleString("en-US") + " " + monthPnL;
    //document.getElementById("monthly").innerHTML = "$"+ wellRMPL.toLocaleString("en-US")+" "+ monthPnL;
    document.getElementById("YTD").innerHTML = "$" + wellYTDPL.toFixed(0).toLocaleString("en-US") + " YTD";
  })
...
//LINEAR LISTENERS//
d3.select("#linear").on('click', function () { Curve(timeFrame = 0, scale = 'linear'); });
d3.select("#Inception").on('click', function () { Curve(timeFrame = 0, scale = 'linear'); });
d3.select("#Days30").on('click', function () { Curve(timeFrame = 31, scale = 'linear'); });
d3.select("#Days180").on('click', function () { Curve(timeFrame = 181, scale = 'linear'); });
d3.select("#Days365").on('click', function () { Curve(timeFrame = 366, scale = 'linear'); });

//LOG LISTENERS//
d3.select("#logarithmic").on('click', function () { Curve(timeFrame = 0, scale = 'log'); });
d3.select("#InceptionLog").on('click', function () { Curve(timeFrame = 0, scale = 'log'); });
d3.select("#Days30Log").on('click', function () { Curve(timeFrame = 31, scale = 'log'); });
d3.select("#Days180Log").on('click', function () { Curve(timeFrame = 181, scale = 'log'); });
d3.select("#Days365Log").on('click', function () { Curve(timeFrame = 366, scale = 'log'); });

// LOG LISTENERS FROM ANALYZE //DONT HAVE 
//d3.select("#logarithmic").on('load', function() {Curve(timeFrame=0,scale='log');});
//$( "#logarithmic" ).on( "click", function() {ClickedFromAnalyze(timeFrame=0,scale='log');}  );
