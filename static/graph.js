...
function Curve(timeFrame, scale) {
    ...
    } else if (sessionStorage.getItem("siteSelection") != null) {//redirected from anaylze
...
d3.select("#siteSelection").on('change', () => { Curve(timeFrame = 0, scale = 'linear'); });

//LINEAR LISTENERS//
d3.select("#linear").on('click', () => { Curve(timeFrame = 0, scale = 'linear'); });
d3.select("#Inception").on('click', () => { Curve(timeFrame = 0, scale = 'linear'); });
d3.select("#Days30").on('click', () => { Curve(timeFrame = 31, scale = 'linear'); });
d3.select("#Days180").on('click', () => { Curve(timeFrame = 181, scale = 'linear'); });
d3.select("#Days365").on('click', () => { Curve(timeFrame = 366, scale = 'linear'); });

//LOG LISTENERS//
d3.select("#logarithmic").on('click', () => { Curve(timeFrame = 0, scale = 'log'); });
d3.select("#InceptionLog").on('click', () => { Curve(timeFrame = 0, scale = 'log'); });
d3.select("#Days30Log").on('click', () => { Curve(timeFrame = 31, scale = 'log'); });
d3.select("#Days180Log").on('click', () => { Curve(timeFrame = 181, scale = 'log'); });
d3.select("#Days365Log").on('click', () => { Curve(timeFrame = 366, scale = 'log'); });

//TABLE LISTENER//
d3.select("#table").on('click', () => { table() });
